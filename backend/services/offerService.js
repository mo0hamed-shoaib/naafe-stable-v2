...
class OfferService {
  ...
  async createOffer(jobRequestId, providerId, offerData) {
    try {
      ...
      // Validate price is within budget - only if both job request and offer have budget
      if (jobRequest.budget && jobRequest.budget.min && jobRequest.budget.max && offerData.budget) {
  ...
  async updateOffer(offerId, providerId, updateData) {
    try {
      ...
      const allowedUpdates = ['budget', 'message', 'selectedScheduleItems'];
